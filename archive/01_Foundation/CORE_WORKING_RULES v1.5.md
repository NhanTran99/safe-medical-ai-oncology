...
WR-010 — Two-Part Population Package Delivery

Sau LOCK:
Part 1 = CKO + KP
Part 2 = PEP + QA + Registry
Không hỏi lại giữa hai phần.

WR-010A — Batch Artifact Completion Rule

Once a Discussion Batch is LOCKED:
Immediately generate Part 1 (CKO + Knowledge Passport).
Continue directly with Part 2 (Evidence Package + QA + Registry Update) in subsequent responses without requesting confirmation.
After completion, automatically prepare the next Discussion Batch if no architectural blocker exists.

WR-010B — Final Response Status Rule

The final response of every completed Population Package shall include the Population Registry Update followed by the standardized execution status block (Current Phase, Current Step, Completed Artifacts, Project Progress, Next Discussion Batch, Next Artifact).

WR-010C — Standard Execution Status Block

Every execution response shall conclude with a standardized status block containing:
...
This standardized block shall be used consistently across all Population Package execution threads to support long-term continuity and thread handover.

WR-010D — Discussion Batch Continuity Rule

During Population Package production,
after completion of Part 2,
the Strategist shall automatically continue by preparing the next Discussion Batch
unless an architectural blocker,
scope conflict,
or duplicate Population Package is identified.

This preserves uninterrupted execution across long Population Waves.
...
The Project Coordinator's explicitly provided Population Package list/sequence is the execution source of truth for which Population Package is worked on next.
...
WR-015 — Immediate Complete Population Package Artifact Rule
...
when these have already been established by the Gold Population Package Specification and project Source Files.
