...
# ============================================================
# PHASE 3C — LAYER 3 AGGREGATE ARTIFACT AUDIT v1.4
...
heading_names <- function(text) {
  ...
  unique(vapply(h, normalize_heading, character(1)))
}

heading_present <- function(text, phrase) {
  target <- normalize_heading(phrase)
  any(tolower(heading_names(text)) == tolower(target))
}
...
check_required_sections <- function(text, artifact) {
  ...
  # Governed wording variants that are semantically equivalent.
  if (artifact == "EP" && !present[required == "Out-of-Scope Topics"]) {
    present[required == "Out-of-Scope Topics"] <-
      heading_present(text, "Out-of-Scope Topics / Delegated Packages")
  }
...
for (i in seq_along(EXPECTED_PP_IDS)) {
  ...
  # Cross-artifact consistency is verified from the artifacts themselves,
  # not from exact wording in the QA prose.
  cross_artifact_ok <- all(
    heading_present(texts$QA, "Cross-Artifact Consistency Check"),
    ids_ok,
    version_ok,
    title_ok
  )
...
detail_file <- file.path(OUTPUT_DIR, "L3_Aggregate_Audit_Detail_v1.4.csv")
depth_file <- file.path(OUTPUT_DIR, "L3_Aggregate_Audit_Depth_Metrics_v1.4.csv")
exception_file <- file.path(OUTPUT_DIR, "L3_Aggregate_Audit_Exceptions_v1.4.csv")
summary_file <- file.path(OUTPUT_DIR, "L3_Aggregate_Audit_Summary_v1.4.txt")
...
cat("\n============================================================\n")
cat("LAYER 3 AGGREGATE ARTIFACT AUDIT v1.4 — COMPLETED\n")
cat("============================================================\n")
