# ============================================================
# PHASE 3 COMPLETION & PHASE 4 READINESS AUDIT v1.3
# ============================================================
# READ-ONLY audit.
#
# Purpose:
...
manifest_candidates <- c(
  file.path(REPO_ROOT, "working/POPULATION_PACKAGE_INTEGRATION_MANIFEST_completed.xlsx"),
...
EXPECTED_COMMIT <- "a838a9423fc3d14c46f8cd176bafed3b691e65c0"
...
if (!is.null(manifest) && !is.na(manifest_id_col)) {
  ...
  for (i in seq_len(nrow(manifest_rows))) {
    ...
    if (!nzchar(mp) || !nzchar(fp)) {
      # Fixed-string path comparison; no regex escaping.
      folder_base <- basename(fp2)
      manifest_base <- basename(mp2)

      manifest_rows$Path_Matches_Folder[i] <-
        identical(mp2, fp2) ||
        identical(manifest_base, folder_base) ||
        grepl(folder_base, mp2, fixed = TRUE) ||
        grepl(fp2, mp2, fixed = TRUE)
    }
  }
}
...
fixed_to_regex <- function(x) {
  # Escape regex metacharacters safely using character-wise substitution.
  chars <- strsplit(x, "", fixed = TRUE)[[1]]
  specials <- c(".", "\\", "^", "$", "*", "+", "?", "(", ")", "[", "]", "{", "}", "|")
  paste0(
    vapply(chars, function(ch) {
      if (ch %in% specials) paste0("\\", ch) else ch
    }, character(1)),
    collapse = ""
  )
}
...
run_cmd <- function(cmd) {
  # Quote the repository path because it contains a space ("GIT hub").
  # cmd is otherwise passed unchanged.
  if (length(cmd) >= 2L && identical(cmd[1], "-C")) {
    cmd[2] <- shQuote(cmd[2], type = "cmd")
  }
  tryCatch(
...
git_artifact_counts <- data.frame(
  ...
  Count = vapply(EXPECTED_FILES, function(f)
    sum(grepl(paste0("/", fixed_to_regex(f), "$"),
...
git_remote_pass <- any(
  grepl(EXPECTED_REMOTE, git_remote, fixed = TRUE)
)
...
summary_lines <- c(
  ...
  paste0("Expected current Phase 3 commit: ", EXPECTED_COMMIT),
