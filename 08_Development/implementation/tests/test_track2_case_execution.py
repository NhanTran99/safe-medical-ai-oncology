...
def test_cer_evaluate_executes_pp_0001_via_ec_0001():
    response = client.post("/cer/evaluate", json={"request_text": "test question", "case_id": "EC-0001"})
...
def test_cer_evaluate_executes_pp_0002_via_ec_0002():
    response = client.post("/cer/evaluate", json={"request_text": "test question", "case_id": "EC-0002"})
...
def test_cer_evaluate_executes_pp_0003_via_ec_0003():
    response = client.post("/cer/evaluate", json={"request_text": "test question", "case_id": "EC-0003"})
...
def test_cer_evaluate_executes_pp_0147_via_ec_0147():
    response = client.post("/cer/evaluate", json={"request_text": "test question", "case_id": "EC-0147"})
...
def test_cer_evaluate_executes_pp_0239_via_ec_0239():
    response = client.post("/cer/evaluate", json={"request_text": "test question", "case_id": "EC-0239"})
...
def test_different_cases_retrieve_the_correct_distinct_pp_artifact():
    ...
    for case_id, expected_pp in expectations.items():
        result = _run_controlled_evaluation(
            ControlledEvaluationRequest(request_text="test question", case_id=case_id)
        )
...
def test_cer_evaluate_ignores_an_extraneous_population_id_field():
    ...
    response = client.post(
        "/cer/evaluate",
        json={"request_text": "test question", "case_id": "EC-0003", "population_id": "PP-0002"},
    )
    assert response.status_code == 200
    body = response.json()
    assert body["outcome"] == "COMPLETED"
